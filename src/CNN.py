...
class SpectrumAutoencoder(Model):
    def __init__(self, latent_dim, input_shape):
        ...
        self.decoder = tf.keras.Sequential([
            layers.Dense((input_shape[0] // 4) * 64, activation='relu'),
            layers.Reshape((input_shape[0] // 4, 64)),
            ...
            layers.UpSampling1D(size=2),
            layers.Conv1DTranspose(input_shape[1], kernel_size=3, activation='sigmoid', padding='same')
        ])
